...
redisClient *createClient(int fd) {
    ...
    /* lig: set default replication&monitor option */
    c->enablerepl = 1;
    c->enablemonitor = 1;
    return c;
}
...
/* lig: replication&monitor enable/disable function */
void enableRepl(redisClient *c) {
    int enable = atoi(c->argv[1]->ptr);
    c->enablerepl = enable;
    addReply(c,shared.ok);
}

void enableMonitor(redisClient *c) {
    int enable = atoi(c->argv[1]->ptr);
